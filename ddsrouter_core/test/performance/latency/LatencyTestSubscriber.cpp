...
LatencyTestSubscriber::~LatencyTestSubscriber()
{
    ...
    {
        logError(LatencyTest_Subscriber, "ERROR removing the endpoints");
    }
    if (!latency_data_type_)
    {
        logError(LatencyTest_Subscriber, "ERROR unregistering the latency data type")
    }
        ...
        LatencyDataSizes& latency_data_sizes)
{
    ...
    {
        logError(LatencyTest_Subscriber, "ERROR creating PUBLISHER");
        return false;
    }
    ...
    {
        logError(LatencyTest_Subscriber, "ERROR creating SUBSCRIBER");
        return false;
    }

    {
        ...
        if (xml_config_file_.length() > 0 )
        {
            ...
            {
                logError(LatencyTest_Subscriber, "ERROR unable to retrieve the " << pub_profile_name << "from XML file");
                return false;
            }

            if ( ReturnCode_t::RETCODE_OK != subscriber_->get_datareader_qos_from_profile(sub_profile_name, dr_qos_))
            {
                logError(LatencyTest_Subscriber, "ERROR unable to retrieve the " << sub_profile_name);
                return false;
            }
        }
        // Create QoS Profiles
        ...
        DataReader* reader)
{
    ...
    // latency_data_type do not require locks because the command message
    // exchange assures this calls atomicity

    SampleInfo info;
    void* data = static_cast<void*>(sub->latency_data_);
        ...
        uint32_t datasize)
{
    ...
    if (init_static_types(datasize) && create_data_endpoints())
    {
        ...
        // Wait for new endpoints discovery from the LatencyTestPublisher
        wait_for_discovery(
            [this]() -> bool
            {
        ...
        uint32_t payload)
{
    ...
    if (latency_data_type_)
    {
        logError(LatencyTest_Subscriber, "ERROR STATIC DATA type already initialized");
        return false;
    }
    else if (participant_->find_type(LatencyDataType::type_name_))
    {
        logError(LatencyTest_Subscriber, "ERROR STATIC DATA type already registered");
        return false;
    }
    ...
    {
        logError(LatencyTest_Subscriber, "ERROR registering the STATIC DATA type");
        return false;
    }
