...
/**
 * @file LatencyTest_Publisher.cpp
 *
 */
...
LatencyTestPublisher::~LatencyTestPublisher()
{
    ...
    {
        logError(LatencyTest_Publisher, "ERROR removing the endpoints")
    }
    if (!latency_data_type_)
    {
        logError(LatencyTest_Publisher, "ERROR unregistering the latency data type")
    }
    ...
    logDebug(LatencyTest, "Pub: Participant removed");
}
        ...
        LatencyDataSizes& latency_data_sizes)
{
    ...
    *output_files_[AVERAGE_INDEX] << std::endl;
    ...
    {
        logError(LatencyTest_Publisher, "ERROR registering the COMMAND type");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR creating PUBLISHER");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR creating SUBSCRIBER");
        return false;
    }
    ...
    /* Create Data DataWriter and DataReader QoS Profile*/
    {
        ...
        if (xml_config_file_.length() > 0)
        {
            ...
            {
                logError(LatencyTest_Publisher, "ERROR unable to retrive the " << pub_profile_name << "from XML file");
                return false;
            }

            if (ReturnCode_t::RETCODE_OK != subscriber_->get_datareader_qos_from_profile(sub_profile_name, dr_qos_))
            {
                logError(LatencyTest_Publisher, "ERROR unable to retrive the " << sub_profile_name << "from XML file")
            }
        }
        // Create QoS Profiles
        else
        {
            ...
            if (reliable)
            {
                ...
                RTPSReliableWriterQos reliable_dw_qos;
                reliable_dw_qos.times.heartbeatPeriod.seconds = 0;
                reliable_dw_qos.times.heartbeatPeriod.nanosec = 100000000;
                dw_qos_.reliable_writer_qos(reliable_dw_qos);
            }
            else
            {
    ...
    // Static actual data types and endpoints are created for each payload iteration

    return true;
}

/*
 * Our current implementation of MatchedStatus info:
        ...
        const PublicationMatchedStatus& info)
{
    ...
    {
        logDebug(LatencyTest, C_MAGENTA << "Data Pub Matched" << C_DEF);
    }
        ...
        const SubscriptionMatchedStatus& info)
{
    ...
    if (info.current_count_change > 0)
    {
        logDebug(LatencyTest, C_MAGENTA << "Data Sub Matched" << C_DEF);
    }
        ...
        const PublicationMatchedStatus& info)
{
    ...
    if (info.current_count_change > 0)
    {
        logDebug(LatencyTest, C_MAGENTA << "Command Pub Matched" << C_DEF);
    }
        ...
        const SubscriptionMatchedStatus& info)
{
    ...
    if (info.current_count_change > 0)
    {
        logDebug(LatencyTest, C_MAGENTA << "Command Sub Matched" << C_DEF);
    }
        ...
        DataReader* reader)
{
    ...
    SampleInfo info;
    void* data = static_cast<void*>(pub->latency_data_in_);
        ...
        uint32_t datasize)
{
    ...
    if (init_static_types(datasize) && create_data_endpoints())
    {
    ...
    {
        logError(LatencyTest_Publisher, "Error preparing types and endpoints for testing");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "Endpoints for payload size " << datasize << " could not been removed");
        return false;
    }
        ...
        uint32_t payload)
{
    ...
    if (latency_data_type_)
    {
        logError(LatencyTest_Publisher, "ERROR STATIC DATA type already initialized");
        return false;
    }
    else if (participant_->find_type(LatencyDataType::type_name_))
    {
        logError(LatencyTest_Publisher, "ERROR STATIC DATA type already registered");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR registering the STATIC DATA type");
        return false;
    }
...
bool LatencyTestPublisher::create_data_endpoints()
{
    ...
    {
        logError(LatencyTest_Publisher, "ERROR latency_data_pub_topic_ already initialized");
        return false;
    }

    if (nullptr != latency_data_sub_topic_)
    {
        logError(LatencyTest_Publisher, "ERROR latency_data_sub_topic_ already initialized");
        return false;
    }

    if (nullptr != data_writer_)
    {
        logError(LatencyTest_Publisher, "ERROR data_writer_ already initialized");
        return false;
    }

    if (nullptr != data_reader_)
    {
        logError(LatencyTest_Publisher, "ERROR data_reader_ already initialized");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR creating latency_data_pub_topic_");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR creating latency_data_sub_topic_");
        return false;
    }
...
bool LatencyTestPublisher::destroy_data_endpoints()
{
    ...
    {
        logError(LatencyTest_Publisher, "ERROR destroying the DataWriter");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR destroying the DataReader");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR destroying the DATA PUB topic");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR destroying the DATA SUB topic");
        return false;
    }
    ...
    {
        logError(LatencyTest_Publisher, "ERROR unregistering the DATA type");
        return false;
    }
